...
(
	...
	(
		...
		(
			...
			'child_record_callback'   => array('tl_catalog_category', 'generateItemRow')
		),
		'label' => array
		(
			'fields'                  => array('title'),
			'format'                  => '%s',
			//'label_callback'          => array('tl_catalog_category', 'addProductsQTY')
		),
		...
		(
			...
			'toggle' => array
			(
				'label'               => &$GLOBALS['TL_LANG']['tl_catalog_category']['toggle'],
				'icon'                => 'visible.gif',
				'attributes'          => 'onclick="Backend.getScrollOffset();return AjaxRequest.toggleVisibility(this,%s)"',
				'button_callback'     => array('tl_catalog_category', 'toggleIcon')
			),
			'show' => array
			(
	...
	(
		...
		'alias' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_category']['alias'],
			'exclude'                 => true,
			'search'                  => true,
			'inputType'               => 'text',
		...
		'addImage' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_category']['addImage'],
			'exclude'                 => true,
			'inputType'               => 'checkbox',
		...
		'singleSRC' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_category']['singleSRC'],
			'exclude'                 => true,
			'inputType'               => 'fileTree',
		...
		'published' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_category']['published'],
			'exclude'                 => true,
			'filter'                  => true,
			'flag'                    => 1,
			'inputType'               => 'checkbox',
...
class tl_catalog_category extends Backend {
	...
	public function toggleVisibility($intId, $blnVisible)
	{
		...
		$this->createInitialVersion('tl_catalog_category', $intId);

		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_catalog_category']['fields']['published']['save_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_catalog_category']['fields']['published']['save_callback'] as $callback)
			{
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_catalog_category SET tstamp=". time() .", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")
					   ->execute($intId);

		$this->createNewVersion('tl_catalog_category', $intId);

	}

	public function addProductsQTY($row, $label) {

        $objChildren = $this->Database->prepare("SELECT COUNT(*) AS count FROM tl_catalog_product WHERE pid=?")
                ->execute($row['id']);

        $label .= ' <span style="color:#b3b3b3; padding-left:3px;">' . sprintf('[%s ' . $GLOBALS['TL_LANG']['tl_catalog_category']['products'] . ']', $objChildren->count) . '</span>';
